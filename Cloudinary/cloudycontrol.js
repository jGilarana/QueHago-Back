...
async function getImage(req, res) {
  ...
  try {
    ...
    const result = await cloudinary.api.resource(
      "5d3daa5dfa02609d6e2acb1619693d1a_fqmwp1",
      options
    );
    console.log(result);
...
async function postEventImage(req, res) {
  ...
  try {
    ...
    const result = await cloudinary.uploader.upload(req.file.path, options);
    console.log(result);
    const [event, eventExists] = await Event.update(
      { image: result.url },
      {
...
async function postClubImage(req, res) {
  ...
  try {
    ...
    const result = await cloudinary.uploader.upload(req.file.path, options);
    console.log(result);
    const [event, eventExists] = await Club.update(
      { image: result.url },
      {
        where: { id: req.params.id },
      }
...
async function postUserImage(req, res) {
  ...
  try {
    ...
    const [_, [event]] = await User.update(
      { image: result.url },
      {
